...
public class HttpRequestSender extends JFrame {
    ...
    public HttpRequestSender() {
        ...
        methodLabel.setBounds(20, 140, 100, 25);
        add(methodLabel);

        methodComboBox = new JComboBox<>(new String[]{"POST", "GET"});
        methodComboBox.setBounds(150, 140, 100, 25);
        add(methodComboBox);

        JLabel contentTypeLabel = new JLabel("Content-Type:");
        contentTypeLabel.setBounds(20, 180, 100, 25);
        add(contentTypeLabel);

        contentTypeField = new JTextField();
        contentTypeField.setBounds(150, 180, 300, 25);
        add(contentTypeField);

        sendButton = new JButton("Send Request");
        sendButton.setBounds(150, 220, 150, 30);
        add(sendButton);

        statusLabel = new JLabel("");
        statusLabel.setBounds(20, 260, 500, 25);
        add(statusLabel);
    ...
    private void sendHttpRequest() {
        ...
        try {
            ...
            // Gửi dữ liệu (nếu là POST)
            if ("POST".equals(method)) {
                try (OutputStream os = conn.getOutputStream()) {
                    byte[] input = bodyData.getBytes("Shift_JIS");  // Nếu dữ liệu cần mã hóa bằng Shift_JIS
